...
import { ArrowLeft, Trash2, Zap } from "lucide-react-native";
...
export default function PositiveEntriesScreen() {
  const { entries, loading, refetch } = useEnergyEntries();
  const [deletingIds, setDeletingIds] = useState<Set<string>>(new Set());

  // Filter only energizing entries
  const positiveEntries = entries.filter(
    (entry) => entry.type === "energizing"
  );
      ...
      >
        <View style={styles.entryHeader}>
          <View style={styles.entryTypeContainer}>
            ...
            <ThemedText style={styles.entryType}>Energizing</ThemedText>
          </View>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <ThemedText style={styles.loadingText}>
              Loading your positive entries...
            </ThemedText>
          </View>
        ) : positiveEntries.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Zap size={48} color="#16a34a" />
            <ThemedText style={styles.emptyTitle}>
              No Energizing Activities Yet
            </ThemedText>
            <ThemedText style={styles.emptyText}>
              Start tracking activities that boost your energy to see them here!
            </ThemedText>
          </View>
        ) : (
          <FlatList
            data={positiveEntries}
